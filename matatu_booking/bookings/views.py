...
def create_booking(request, route_id):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            messages.error(request, "Seat was just taken. Try again.")
            return redirect(f"/book/{route.id}/?bus={bus.id}")
        ...
        try:
            ...
            if response.get("ResponseCode") != "0":
                ...
                messages.error(request, "M-Pesa request failed. Try again.")
                return redirect(f"/book/{route.id}/?bus={bus.id}")
            ...
            if not checkout_id:
                ...
                messages.error(request, "Invalid M-Pesa response.")
                return redirect(f"/book/{route.id}/?bus={bus.id}")
        ...
        messages.success(request, "STK push sent. Complete payment.")
...
def download_ticket(request, booking_id):
    ...
    elements.append(Paragraph("MATATU TICKET", styles["Title"]))
    elements.append(Spacer(1, 20))

    elements.append(Paragraph(f"<b>Reference:</b> {booking.reference}", styles["Normal"]))
    elements.append(Paragraph(f"<b>Passenger:</b> {booking.passenger_name}", styles["Normal"]))
    elements.append(Paragraph(f"<b>Route:</b> {booking.route}", styles["Normal"]))
    elements.append(Paragraph(f"<b>Seat:</b> {booking.seat_number}", styles["Normal"]))
    elements.append(Paragraph(f"<b>Status:</b> {booking.status}", styles["Normal"]))

    if payment:
        elements.append(
            Paragraph(f"<b>M-Pesa Receipt:</b> {payment.merchant_request_id}", styles["Normal"])
        )
